...
<!-- SimpleMDE css -->
  ...
  <link href="{{asset('admin/css/colorbox.css')}}" rel="stylesheet">
@endpush
...
<div class="content-page">
  <div class="content">
      ...
      <form action="{{route('dashboard.post.store')}}" method="POST" enctype="multipart/form-data" class="container-fluid">
          <div class="row mt-3">
              ...
              <div class="card-block p-3" style="background: #fff" >
                    ...
                    <select class="form-control select2" name="item_id" data-toggle="select2">
                      @foreach ($items as $item)
                          <option value="{{$item->id}}">{{$item->name_qr}}</option>
                      @endforeach
                    </select>
                        ...
                        @foreach (config('app.available_locales') as $local)
                          <div class="tab-pane  <?=($local['lang']=='ru' ? 'show active' : '')?>" id="item_{{$local['lang']}}">
                            <div class="mb-3">
                              ...
                              ?>
                              <input type="text" name="title_{{$local['lang']}}" value="{{old('title_'.$local['lang'])}}" id="simpleinput" class="form-control">
                            </div>
                            <div class="col-md-12 mt-3">
                                <div class="form-group">
                                    <label for="body">{{$local['body']}}</label>
                                    <textarea class="form-control" id="editor_{{$local['lang']}}" name="body_{{$local['lang']}}" aria-valuemax="{{old('body_'.$local['lang'])}}" rows="10" placeholder="Введите текст" ></textarea>
                                </div>
                            </div>
                          </div>
                        @endforeach
...
<script type="text/javascript" src="{{asset('admin/js/jquery.colorbox-min.js')}}"></script>
